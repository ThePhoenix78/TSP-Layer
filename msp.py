...
def deep_search(G, node0: int = 0, weights: int = 0):
    ...
    G.nodes[node0]["nb_visit"] += 1
    len_nodes = len(G.nodes)
    len_neighs = len(neighs)

    for node1 in neighs:
        if len_neighs > 1:
            if G[node0][node1].get("nb_cross") > len_nodes//3 or G.nodes[node1].get("nb_visit") > len_neighs:
                continue
...
G = create_graph(8, seed=2, fully_connected=True)
